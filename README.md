...
Clone this repository and simply launch `media-autobuild_suite.bat` to build an exact replica of `ffmpeg.exe` that is bundled with [Notessimo](https://notessimo.net), the binary are then available at `local64/bin-video/ffmpeg.exe`.
